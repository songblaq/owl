...
5. **Draft summary** with all required headers. Save to `compiled/YYYY-MM-DD-<slug>-summary.md` matching the raw file's slug.
6. **Optionally draft note** if there's interpretation worth recording.
7. **Add cross-links**: at least 1 link to an existing compiled doc if related work exists.
8. **Run `owl health --json`** to verify no new health issues introduced.
9. **Report** what you created with paths and 1-line summaries.
