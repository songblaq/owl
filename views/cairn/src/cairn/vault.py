"""Vault discovery for cairn.

cairn-vault is a local directory marked by a ``.cairn-vault`` file. Discovery
order (first hit wins):

1. ``--vault`` CLI flag
2. ``$CAIRN_VAULT`` environment variable
3. ``~/.cairn/active-vault`` config pointer
4. Walk up from cwd looking for a ``.cairn-vault`` marker
5. Default: ``~/cairn-vault``
"""
...
MARKER_FILE = ".cairn-vault"
USER_CONFIG_DIR = Path.home() / ".cairn"
ACTIVE_VAULT_FILE = USER_CONFIG_DIR / "active-vault"
DEFAULT_VAULT = Path.home() / "cairn-vault"
...
def discover_vault(vault_arg: Optional[str] = None) -> Path:
    ...
    # 2. env var
    env = os.environ.get("CAIRN_VAULT")
    if env:
...
def discovery_source(vault_arg: Optional[str] = None) -> str:
    ...
    if os.environ.get("CAIRN_VAULT"):
        return "$CAIRN_VAULT env var"
    if ACTIVE_VAULT_FILE.exists():
