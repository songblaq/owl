"""Vault discovery for facet.

facet-vault is a local directory marked by a ``.facet-vault`` file. Discovery
order (first hit wins):

1. ``--vault`` CLI flag
2. ``$FACET_VAULT`` environment variable
3. ``~/.facet/active-vault`` config pointer
4. Walk up from cwd looking for a ``.facet-vault`` marker
5. Default: ``~/facet-vault``
"""
...
MARKER_FILE = ".facet-vault"
USER_CONFIG_DIR = Path.home() / ".facet"
ACTIVE_VAULT_FILE = USER_CONFIG_DIR / "active-vault"
DEFAULT_VAULT = Path.home() / "facet-vault"
...
def discover_vault(vault_arg: Optional[str] = None) -> Path:
    ...
    # 2. env var
    env = os.environ.get("FACET_VAULT")
    if env:
...
def discovery_source(vault_arg: Optional[str] = None) -> str:
    ...
    if os.environ.get("FACET_VAULT"):
        return "$FACET_VAULT env var"
    if ACTIVE_VAULT_FILE.exists():
...
def iter_shards(vault: Path):
    """Yield (shard_name, shard_path) for each shard directory under vault/shards/."""
    shards_dir = vault / "shards"
    if not shards_dir.exists():
        return
    for d in sorted(shards_dir.iterdir()):
        if d.is_dir():
            yield d.name, d
