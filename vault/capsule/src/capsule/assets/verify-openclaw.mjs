...
function parseArgs(argv) {
  /** @type {{ includeI18n: boolean; openclawRoot?: string; distRoot?: string }} */
...
async function main() {
  ...
  const currentHead = safeGit(["rev-parse", "HEAD"], openclawRoot);
  const manifestHead = manifest?.openclaw?.gitHead ?? null;
  if (manifestHead && currentHead && manifestHead !== currentHead) {
    console.error(`OpenClaw git head changed: manifest=${manifestHead} current=${currentHead}`);
    console.error("Rebuild the dist pack to match the current OpenClaw checkout.");
    process.exitCode = 1;
    return;
  }
