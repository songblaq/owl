...
function parseArgs(argv) {
  /** @type {{ includeI18n: boolean; sourceRoot?: string; distRoot?: string; maxCharsPerPart: number }} */
...
function renderLlmsTxt(corePages) {
  const lines = [];
  lines.push("# Hermes Docs — Capsule (compact)");
  lines.push("");
  lines.push("Lossless, LLM-friendly mirror of `hermes/docs/**`.");
  ...
  lines.push("");
  lines.push("## Delta");
  lines.push("- delta/delta-YYYY-MM-DD.md (only when Hermes git head changes)");
  lines.push("");
  lines.push("## Index");
  lines.push("- manifest.json (source/output hashes + Hermes git head)");
  lines.push("");
  return lines.join("\n");
}
...
function renderLlmsFullTxt(corePages) {
  const lines = [];
  lines.push("# Hermes Docs — Capsule (full)");
  lines.push("");
  lines.push("Lossless, LLM-friendly mirror of `hermes/docs/**`.");
...
/**
 * @param {{
 *   hermesGitHead: string | null;
 *   hermesGitDescribe: string | null;
 *   includeI18n: boolean;
 * }} opts
 */
function renderAtlas(opts) {
  const lines = [];
  lines.push("# Hermes Capsule Atlas (compact)");
  lines.push("");
  lines.push(
    "This folder is for LLM agents. It mirrors Hermes docs with layout stripped but content preserved.",
  );
  ...
  lines.push("");
  if (opts.hermesGitDescribe || opts.hermesGitHead) {
    lines.push("Hermes source:");
    if (opts.hermesGitDescribe) lines.push(`- gitDescribe: ${opts.hermesGitDescribe}`);
    if (opts.hermesGitHead) lines.push(`- gitHead: ${opts.hermesGitHead}`);
    lines.push("");
  }
  ...
  lines.push("- INDEX_CLI_COMMANDS.md (lines that start with `hermes ...`)");
  lines.push("");
...
function renderAtlasFull(opts) {
  const lines = [];
  lines.push("# Hermes Capsule Atlas (full)");
  lines.push("");
  lines.push(
    "This folder is for LLM agents. It mirrors Hermes docs with layout stripped but content preserved.",
  );
  lines.push("Navigation below is derived from `hermes/docs/docs.json`.");
  lines.push("");
  if (opts.hermesGitDescribe || opts.hermesGitHead) {
    lines.push("Hermes source:");
    if (opts.hermesGitDescribe) lines.push(`- gitDescribe: ${opts.hermesGitDescribe}`);
...
function sourcePathToPagesRel(sourcePath) {
  if (sourcePath === "hermes/CHANGELOG.md") {
    return "extra/CHANGELOG.md";
  }
  if (sourcePath.startsWith("hermes/docs/")) {
    const relFromDocs = sourcePath.slice("hermes/docs/".length);
...
async function main() {
  ...
  const sourceRoot = path.resolve(
    toolsRoot,
    args.sourceRoot ? args.sourceRoot : "../hermes-agent",
  );
  const distRoot = path.resolve(toolsRoot, args.distRoot ? args.distRoot : "../hermes-agent-docs-capsule");
  ...
  const configIndexText = renderIndexMarkdown(configKeyIndex, {
    ...
    description:
      "Extracted from backticked dotted tokens in `pages/**` and filtered to common Hermes config roots. Open the referenced page for full context.",
  });
  ...
  const cliIndexText = renderIndexMarkdown(cliCommandIndex, {
    ...
    description:
      "Extracted from lines that start with `hermes ...` in `pages/**`. Listed by top-level command. Open the referenced page for full context.",
  });
  ...
  const manifest = {
    tool: { name: "hermes-docs-capsule", version: AI_PACK_VERSION },
    maxCharsPerPart: args.maxCharsPerPart,
    hermes: {
      gitHead: hermesGitHead,
      gitDescribe: hermesGitDescribe,
    },
    inputs: {
      docsRoot: "hermes/docs",
      changelog: hasChangelog ? "hermes/CHANGELOG.md" : null,
      includeI18n: args.includeI18n,
    },
  ...
  // Delta (only when Hermes git head changed and we can compute a range).
  const prevHead = prevManifest?.hermes?.gitHead ?? null;
  const nextHead = hermesGitHead ?? null;
  let deltaPath = null;
  ...
  if (prevHead && nextHead && prevHead !== nextHead) {
    ...
    try {
      const raw = git(["diff", "--name-status", `${prevHead}..${nextHead}`, "--", "docs", "CHANGELOG.md"], sourceRoot);
    ...
    changedSources = {
      ...
      modified,
      deleted,
      otherChangedPaths: nonTextChanges.map((p) => `hermes/${p}`),
    };

    const lines = [];
    lines.push(`# Hermes Docs LLM Pack Delta (${stamp})`);
    lines.push("");
    lines.push(`Hermes: ${prevHead}..${nextHead}`);
    lines.push("");
