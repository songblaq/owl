...
export function canonicalUrlForDocsPath(relFromDocs) {
  ...
  if (slug.endsWith("/index")) {
    ...
    return `https://docs.openclaw.ai/${base}`;
  }
  return `https://docs.openclaw.ai/${slug}`;
}
...
/**
 * Extract top-level OpenClaw CLI commands from lines like: `openclaw gateway ...`
 * @param {string} text
 */
export function extractCliCommands(text) {
  ...
  for (const line of lines) {
    ...
    // Skip "openclaw@latest"-like strings that may appear in npm commands.
    if (cmd.includes("@")) {
      continue;
    }
    out.add(cmd);
