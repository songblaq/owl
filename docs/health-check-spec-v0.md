...
### 2.5.1 report-missing-output-links 의 exempt 필드

일부 report 는 *텍스트-only 분석* (health check, audit, retrospective 등) 이라 `outputs/*` 산출물이 없는 게 설계 의도. 이 경우 `report-missing-output-links` 룰에서 opt-out 하려면 compiled doc 의 헤더 영역에 다음 marker 를 추가한다:

```
outputs-exempt: true
```

이 marker 가 있으면 `check_report_outputs` 가 해당 report 를 건너뛴다. 구현: `src/owl/health.py` 의 `is_outputs_exempt(content)` 함수.

### 2.5.2 External reference subtree 예외

`raw/` 하위의 *external reference subtree* (예: `raw/atlas/`) 는 owl 의 raw→compiled 계약을 따르지 않는 self-contained knowledge pack 이다. 이들 subtree 의 파일은:

- `missing-summary-for-raw` 등 **raw-based health 룰에서 자동 제외**된다
- 구현: `src/owl/health.py` 의 `EXTERNAL_REFERENCE_SUBTREES` 상수 + `raw_files()` 가 해당 subtree 를 건너뜀
- 대신 `compiled/<name>-external-reference-index.md` 1 파일로 wiki 진입점 제공

자세한 정책은 `docs/folder-policy-v0.md §2` 의 "External reference subtrees" 섹션 참조.
