...
>
> *단, "잘 유지"는 convention이 아니라 enforcement로 달성된다 — 2026-04-17 3-way 실험의 결론.*

LLM 세션이 끝나도 지식이 쌓이는 시스템. 3가지 전략(owl / cairn / wiki)으로 동일 source를 운영하고 benchmark로 승자를 선택한다. 승자는 `omb` CLI에 네이티브로 통합된다.

## 1.1 설계 우선순위 (2026-04-17 확정)

이 시스템이 다뤄야 할 본질적 리스크는 **속도가 아니라 정합성·진실성·추적가능성**이다. 모든 설계 결정은 다음 tier 순으로 이루어진다:

- **Tier 0**: integrity invariants (truth singularity / traceability / immutable source / rebuildable-proven)
- **Tier 1**: enforcement (discipline as code / health fails loudly / merge requires normalize)
- **Tier 2**: quality (atomic / evidence / edges / fresh compile)
- **Tier 3**: performance

상세: `docs/priorities.md`. 충돌 시 상위 tier가 이긴다.
